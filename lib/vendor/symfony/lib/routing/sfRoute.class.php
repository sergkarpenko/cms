...
class sfRoute implements Serializable
{
  ...
  protected function tokenize()
  {
    ...
    while (strlen($buffer))
    {
      if (false !== $this->tokenizeBufferBefore($buffer, $tokens, $afterASeparator, $currentSeparator))
      {
        ...
        {
          if ($afterASeparator && preg_match('#^(' . $this->options['text_regex'] . ')(?:' . $this->options['segment_separators_regex'] . '|$)#', $buffer, $match))
          {
            // a text
          ...
          {
            if (!$afterASeparator && preg_match('#^/|^' . $this->options['segment_separators_regex'] . '#', $buffer, $match))
            {
            ...
            {
              if (false !== $this->tokenizeBufferAfter($buffer, $tokens, $afterASeparator, $currentSeparator))
              {
                // a custom token
                $this->customToken = true;
              }
  ...
  protected function fixSuffix()
  {
    ...
    if ($length > 0 && '/' == $this->pattern[$length - 1])
    {
      ...
      {
        if (preg_match('#\.(?:' . $this->options['variable_prefix_regex'] . $this->options['variable_regex'] . '|' . $this->options['variable_content_regex'] . ')$#i', $this->pattern))
        {
