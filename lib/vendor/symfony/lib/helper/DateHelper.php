  ...
  if ($distance_in_minutes <= 1)
  {
    ...
    else
    {
      if ($distance_in_seconds <= 5)
      {
        ...
        if ($distance_in_seconds >= 6 && $distance_in_seconds <= 10)
        {
          $string = 'less than 10 seconds';
        }
        else
        {
          if ($distance_in_seconds >= 11 && $distance_in_seconds <= 20)
          {
            $string = 'less than 20 seconds';
          }
          else
          {
            if ($distance_in_seconds >= 21 && $distance_in_seconds <= 40)
            {
              $string = 'half a minute';
            }
            else
            {
              if ($distance_in_seconds >= 41 && $distance_in_seconds <= 59)
              {
    ...
    else
    {
      ...
      {
        if ($distance_in_minutes >= 90 && $distance_in_minutes <= 1439)
        {
        ...
        {
          if ($distance_in_minutes >= 1440 && $distance_in_minutes <= 2879)
          {
            $string = '1 day';
          }
          else
          {
            if ($distance_in_minutes >= 2880 && $distance_in_minutes <= 43199)
            {
              $string = '%days% days';
            ...
            {
              if ($distance_in_minutes >= 43200 && $distance_in_minutes <= 86399)
              {
                $string = 'about 1 month';
              }
              else
              {
                if ($distance_in_minutes >= 86400 && $distance_in_minutes <= 525959)
                {
