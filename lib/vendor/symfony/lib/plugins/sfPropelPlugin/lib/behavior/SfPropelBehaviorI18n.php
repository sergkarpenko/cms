...
/**
 * Returns the "
      {$column->getName()}" value from the current {@link
      {$this->getI18nTable()->getPhpName()}}.
 */
public function get
      {$column->getPhpName()}(\$culture = null)
{
  return \$this->getCurrent
      {$refPhpName}(\$culture)->get
      {$column->getPhpName()}();
}

/**
 * Sets the "
      {$column->getName()}" value of the current {@link
      {$this->getI18nTable()->getPhpName()}}.
 *
 * @return
      {$this->getTable()->getPhpName()}
 */
public function set
      {$column->getPhpName()}(\$value, \$culture = null)
{
  \$this->getCurrent
      {$refPhpName}(\$culture)->set
      {$column->getPhpName()}(\$value);
  return \$this;
}
    ...
    {$refPhpName}(\$culture = null)
{
  ...
  {
    \$object = \$this->isNew() ? null :
    {$this->getI18nTable()->getPhpName()}Peer::retrieveByPK(\$this->getPrimaryKey(), \$culture);
    if (\$object)
    {
      \$this->set
    {$refPhpName}ForCulture(\$object, \$culture);
    }
    else
    {
      \$this->set
    {$refPhpName}ForCulture(new
    {$this->getI18nTable()->getPhpName()}(), \$culture);
      \$this->current_i18n[\$culture]->set
    {$this->getI18nTable()->getBehavior('symfony_i18n_translation')->getCultureColumn()->getPhpName()}(\$culture);
    }
  }
 ...
 */
public function set
    {$refPhpName}ForCulture(
    {$this->getI18nTable()->getPhpName()} \$object, \$culture)
{
...
EOF;
  ...
  foreach (sfMixer::getCallables('Base
    {$this->getTable()->getPhpName()}:doSelectJoin:doSelectJoin') as \$sf_hook)
  {
    call_user_func(\$sf_hook, '
    {$this->getTable()->getPhpName()}', \$criteria, \$con);
  }
...
/**
 * Selects a collection of {@link
    {$this->getTable()->getPhpName()}} objects with a {@link
    {$this->getI18nTable()->getPhpName()}} translation populated.
 *
 ...
 * @return array
 */
static public function doSelectWithI18n(Criteria \$criteria, \$culture = null, \$con = null, \$join_behavior = Criteria::
    {$join}_JOIN)
{
  ...
  \$startcol = (
    {$this->getTable()->getPhpName()}Peer::NUM_COLUMNS -
    {$this->getTable()->getPhpName()}Peer::NUM_LAZY_LOAD_COLUMNS);

    {$this->getI18nTable()->getPhpName()}Peer::addSelectColumns(\$criteria);
  \$criteria->addJoin(
    {$this->getLocalColumn()->getConstantName()},
    {$this->getForeignColumn()->getConstantName()}, \$join_behavior);
  \$criteria->add(
    {$this->getCultureColumn($this->getI18nTable())->getConstantName()}, \$culture);
  ...
  \$stmt = BasePeer::doSelect(\$criteria, \$con);
	...
	while (\$row = \$stmt->fetch(PDO::FETCH_NUM)) {
		\$key1 =
    {$this->getTable()->getPhpName()}Peer::getPrimaryKeyHashFromRow(\$row, 0);
		if (null !== (\$obj1 =
    {$this->getTable()->getPhpName()}Peer::getInstanceFromPool(\$key1))) {
    ...
    {$this->getTable()->getPhpName()}Peer::addInstanceToPool(\$obj1, \$key1);
		...
		\$key2 =
    {$this->getI18nTable()->getPhpName()}Peer::getPrimaryKeyHashFromRow(\$row, \$startcol);
		if (\$key2 !== null) {
			\$obj2 =
    {$this->getI18nTable()->getPhpName()}Peer::getInstanceFromPool(\$key2);
			if (!\$obj2) {
				\$cls =
    {$this->getI18nTable()->getPhpName()}Peer::getOMClass(false);
				\$obj2 = new \$cls();
    ...
    {$this->getI18nTable()->getPhpName()}Peer::addInstanceToPool(\$obj2, \$key2);
      ...
      \$obj1->set
    {$refPhpName}ForCulture(\$obj2, \$culture);
