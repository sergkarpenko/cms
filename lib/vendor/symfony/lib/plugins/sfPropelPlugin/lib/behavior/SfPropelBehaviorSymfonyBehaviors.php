...
foreach (sfMixer::getCallables('Base
    {$this->getTable()->getPhpName()}:delete:pre') as \$callable)
{
...
EOF;
  ...
  public function postDelete()
  {
    ...
    return <<<EOF
foreach (sfMixer::getCallables('Base
    {$this->getTable()->getPhpName()}:delete:post') as \$callable)
{
...
EOF;
  ...
  public function preSave()
  {
    ...
    return <<<EOF
foreach (sfMixer::getCallables('Base
    {$this->getTable()->getPhpName()}:save:pre') as \$callable)
{
...
EOF;
  ...
  public function postSave()
  {
    ...
    return <<<EOF
foreach (sfMixer::getCallables('Base
    {$this->getTable()->getPhpName()}:save:post') as \$callable)
{
...
{
  if (!\$callable = sfMixer::getCallable('Base
    {$this->getTable()->getPhpName()}:'.\$method))
  {
    throw new sfException(sprintf('Call to undefined method Base
    {$this->getTable()->getPhpName()}::%s', \$method));
  }
...
static private function getMixerPreSelectHook(\$method)
{
  ...
  {
    return sprintf('Base
    {$this->getTable()->getPhpName()}Peer:%s:%1\$s', 'Count' == \$match[1] ? 'doCount' : \$match[0]);
  }
...
{
  call_user_func(\$sf_hook, 'Base
    {$this->getTable()->getPhpName()}Peer', \$criteria, \$con);
}
...
// symfony_behaviors behavior
    foreach (sfMixer::getCallables('Base
    {$this->getTable()->getPhpName()}Peer:doInsert:pre') as \$sf_hook)
    {
      if (false !== \$sf_hook_retval = call_user_func(\$sf_hook, 'Base
    {$this->getTable()->getPhpName()}Peer', \$values, \$con))
      {
...
// symfony_behaviors behavior
    foreach (sfMixer::getCallables('Base
    {$this->getTable()->getPhpName()}Peer:doUpdate:pre') as \$sf_hook)
    {
      if (false !== \$sf_hook_retval = call_user_func(\$sf_hook, 'Base
    {$this->getTable()->getPhpName()}Peer', \$values, \$con))
      {
...
EOF;
  ...
  public function filterDoInsert($line)
  {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('Base
      {$this->getTable()->getPhpName()}Peer:doInsert:post') as \$sf_hook)
    {
      call_user_func(\$sf_hook, 'Base
      {$this->getTable()->getPhpName()}Peer', \$values, \$con, \$pk);
    }
...
EOF;
  ...
  public function filterDoUpdate($line)
  {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('Base
      {$this->getTable()->getPhpName()}Peer:doUpdate:post') as \$sf_hook)
    {
      call_user_func(\$sf_hook, 'Base
      {$this->getTable()->getPhpName()}Peer', \$values, \$con, \$ret);
    }
